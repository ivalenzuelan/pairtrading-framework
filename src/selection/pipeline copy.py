...
class EnhancedStatisticalAnalyzer:
    ...
    def _enhanced_dtw(self, series1: pd.Series, series2: pd.Series, 
                    asset1: str, asset2: str) -> List[StatisticalResult]:
        """Enhanced Dynamic Time Warping with robust normalization and adaptive scoring."""
        results = []
        
        try:
            # Robust normalization approaches with NaN handling
            normalizations = [
                ("robust_zscore", lambda s: (s - np.nanmedian(s)) / (np.nanstd(s) + 1e-9)),
                ("minmax_quantile", lambda s: (s - np.nanquantile(s, 0.05)) / 
                    (np.nanquantile(s, 0.95) - np.nanquantile(s, 0.05) + 1e-9)),
                ("log_returns", lambda s: np.log(s).diff().fillna(0)),
                ("percent_change", lambda s: s.pct_change().fillna(0).clip(-1, 1))
            ]
            
            for norm_name, norm_func in normalizations:
                try:
                    # Apply normalization with NaN handling
                    ...
                    # Calculate DTW distance with proper alignment
                    dtw_distance = dtw.distance(norm1.values, norm2.values, 
                                            window=int(len(norm1)*0.1),  # 10% warping window
                                            use_pruning=True)  # Optimize calculation
                    
                    # Calculate adaptive max possible distance
                    max_possible = max(len(norm1), len(norm2)) * (np.nanmax(np.abs(norm1)) + np.nanmax(np.abs(norm2)))
                    
                    # Adaptive scoring system
                    if max_possible > 1e-6:  # Avoid division by zero
                        normalized_dist = dtw_distance / max_possible
                        similarity_score = 1 / (1 + 10 * normalized_dist)  # Better scaling
                    else:
                        similarity_score = 0.0
                    
                    # Dynamic significance threshold (1.5% of max possible)
                    is_significant = normalized_dist < 0.015
                    
                    # Calculate volatility-adjusted alignment quality
                    alignment_quality = self._calculate_dtw_alignment_quality(norm1.values, norm2.values)
                    
                    # Volatility ratio metric
                    vol_ratio = min(np.nanstd(norm1), np.nanstd(norm2)) / \
                                max(np.nanstd(norm1), np.nanstd(norm2))
                    ...
                    log.debug(f"DTW {norm_name} failed for {asset1}-{asset2}: {e}")
                    continue
                    
        except Exception as e:
            log.debug(f"DTW analysis failed for {asset1}-{asset2}: {e}")
...
class PairSelector:
    ...
    def select_pairs(self, results: List[StatisticalResult]) -> List[Tuple[str, str]]:
        """Select top pairs based on combined statistical scores."""
        ...
        for result in results:
            ...
            pair_scores[pair]['scores'].append(result.score)
            pair_scores[pair]['total_methods'] += 1
            if result.is_significant:
                pair_scores[pair]['significant_count'] += 1
        ...
        for pair, data in pair_scores.items():
            ...
            # Bonus for having more significant results
            significance_ratio = data['significant_count'] / data['total_methods']
            combined_score = avg_score * (1 + significance_ratio)
            
            pair_rankings.append((pair, combined_score, significance_ratio))
        
        # Sort by combined score and select top pairs
        pair_rankings.sort(key=lambda x: x[1], reverse=True)
        selected_pairs = [pair for pair, score, sig_ratio in pair_rankings[:self.config.max_pairs_per_window]]
        
        log.info(f"Selected {len(selected_pairs)} pairs from {len(pair_rankings)} candidates")
        for i, (pair, score, sig_ratio) in enumerate(pair_rankings[:self.config.max_pairs_per_window]):
            log.info(f"  {i+1}. {pair[0]}-{pair[1]}: score={score:.4f}, significance={sig_ratio:.2f}")
...
class TradingPipeline:
    ...
    def run_training_phase(self, window: WindowResult, data: pd.DataFrame, 
        ...
        # Select best pairs
        window.selected_pairs = self.selector.select_pairs(all_results)
        
        log.info(f"Training completed: analyzed {len(all_results)} pair-method combinations, "
                f"selected {len(window.selected_pairs)} pairs")
    
    def generate_trading_signal(self, pair_data: pd.DataFrame, asset1: str, asset2: str) -> int:
        """Generate trading signal based on price ratio analysis."""
        if len(pair_data) < 5:
            return 0
        ...
        # Generate signal: if ratio is above mean, short asset1/long asset2
        if z_score > 1:
            return -1  # SHORT_LONG
        elif z_score < -1:
            return 1   # LONG_SHORT
    ...
    def run_trading_phase(self, window: WindowResult, data: pd.DataFrame) -> None:
        ...
        if current_date in data.index:
            ...
            for pair in window.selected_pairs:
                ...
                significant_methods = [
                    result.method 
                    for result in window.statistical_results 
                    if result.pair == pair and result.is_significant
                ]
                ...
                if signal != 0:  # Only trade if we have a signal
                    try:
                        position = self.position_manager.open_position(
                            pair, current_date, current_prices, signal, training_methods=significant_methods
                        )
