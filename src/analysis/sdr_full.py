#!/usr/bin/env python3
"""
SDR-based Statistical Arbitrage Backtester
...
DEFAULT_CONFIG = {
    ...
    "sdr_n_components": 10,          # Number of dictionary atoms
    "sdr_alpha": 1.0,                # Sparsity controlling parameter
    "sdr_similarity_threshold": 0.85, # Minimum similarity score
...
# ----------------------------------------------------------------------
# SDR-BASED PAIR SCREENING (STAGE 0 & 1)
# ----------------------------------------------------------------------
def calculate_sdr_similarity(s1: pd.Series, s2: pd.Series, n_components: int, 
                            alpha: float, log: logging.Logger) -> float:
    """Calculate similarity using Sparse Dictionary Representation"""
    try:
        # Create aligned dataframe
        aligned = pd.concat([s1, s2], axis=1).dropna().values
        ...
        dl.fit(aligned)
        codes = dl.transform(aligned)
        
        # Calculate similarity as 1 - cosine distance between code vectors
        sim_score = 1 - cosine(codes[0], codes[1])
        return sim_score
        
    except Exception as e:
        log.debug(f"SDR similarity calculation failed: {e}")
        return 0.0

def screen_sdr_pairs(data: pd.DataFrame, config: dict, window: WindowPhase, 
                     log: logging.Logger) -> Tuple[List[dict], List[dict]]:
    """Stage 0 & 1: SDR-based pair screening"""
    results = []
    valid_pairs = []
    ...
    for (symbol1, col1), (symbol2, col2) in crypto_pairs:
        ...
        try:
            ...
            if actual_points < min_rows:
                log.debug(f"Skipping {symbol1}/{symbol2}: insufficient data ({actual_points} < {min_rows})")
                continue
                
            log.debug(f"Processing {symbol1}/{symbol2}: {actual_points}/{total_points} points")
            
            # Get price series
            s1 = pair_df[col1]
            s2 = pair_df[col2]
            
            # Calculate SDR similarity
            sim_score = calculate_sdr_similarity(
                s1, s2, 
                config["sdr_n_components"], 
                config["sdr_alpha"], 
                log
            )
            
            # Skip if similarity is below threshold
            if sim_score < config["sdr_similarity_threshold"]:
                log.debug(f"Skipping {symbol1}/{symbol2}: low similarity ({sim_score:.4f})")
                continue
            
            # Determine hedge ratio via linear regression
            try:
                lr = LinearRegression()
                lr.fit(s2.values.reshape(-1, 1), s1.values)
                slope = lr.coef_[0]
            ...
            if spread_std < config["min_spread_std"]:
                log.debug(f"Skipping {symbol1}/{symbol2}: spread too stable (std={spread_std:.6f})")
                continue
            
            # Determine lead-lag relationship (using correlation)
            corr_lead = s1.diff().corr(s2.diff().shift(1))  # s1 leads s2
            corr_lag = s1.diff().shift(1).corr(s2.diff())   # s2 leads s1
            
            if corr_lead > corr_lag:
            ...
            log.info(f"Found SDR pair: {leader}->{follower} (sim={sim_score:.4f}, std={spread_std:.4f})")
...
# ----------------------------------------------------------------------
# TRADING EXECUTION (STAGE 2) - SAME AS BEFORE
# ----------------------------------------------------------------------
class PairTrader:
    ...
    def calculate_spread(self, leader_price: float, follower_price: float) -> float:
        ...
        return leader_price - self.hedge_ratio * follower_price - self.intercept
...
# ----------------------------------------------------------------------
# BACKTESTING ENGINE (MODIFIED FOR SDR)
# ----------------------------------------------------------------------
class SDRBacktester:
    """Main backtesting engine for SDR-based strategy"""
    ...
    def run(self):
        ...
        for window in windows:
            ...
            # Screen pairs using SDR method
            try:
                _, valid_pairs = screen_sdr_pairs(
            ...
            required_keys = ["leader", "follower", "hedge_ratio", "intercept", "mean", "std"]
            for pair_info in valid_pairs:
                ...
                try:
                    trader = PairTrader(pair_info, self.config, self.interval, self.log)
...
def main():
    parser = argparse.ArgumentParser(
        description="SDR Statistical Arbitrage Backtester",
    ...
    # Setup logging
    log = setup_logging("sdr_backtester", args.verbose)
    ...
    try:
        ...
        log.info(f"Starting SDR backtest for {args.interval} interval")
        backtester = SDRBacktester(config, repo_root, args.interval, log)
        backtester.run()
