#!/usr/bin/env python3
"""
Correlation-based Statistical Arbitrage Backtester
...
DEFAULT_CONFIG = {
    ...
    "min_correlation": 0.8,          # Minimum correlation threshold
...
# ----------------------------------------------------------------------
# CORRELATION SCREENING (STAGE 0 & 1)
# ----------------------------------------------------------------------
def screen_correlated_pairs(data: pd.DataFrame, config: dict, window: WindowPhase, 
                            log: logging.Logger) -> Tuple[List[dict], List[dict]]:
    """Stage 0 & 1: Correlation screening and Granger causality filtering"""
    results = []
    valid_pairs = []
    ...
    for (symbol1, col1), (symbol2, col2) in crypto_pairs:
        ...
        try:
            ...
            correlation, p_value = pearsonr(s1, s2)
            
            # Skip if correlation is too low
            if abs(correlation) < config["min_correlation"]:
                log.debug(
                    f"Skipping {symbol1}/{symbol2}: low correlation "
                    f"({correlation:.4f})"
                )
                continue
            ...
            # Calculate price ratio instead of spread
            price_ratio = s1 / s2
            ratio_mean = price_ratio.mean()
            ratio_std = price_ratio.std()
            
            # Skip if ratio is too stable
            if ratio_std < config["min_spread_std"]:
                log.debug(
                    f"Skipping {symbol1}/{symbol2}: ratio too stable "
                    f"(std={ratio_std:.6f})"
                )
                continue
            ...
            log.info(
                f"Found correlated pair: {leader}->{follower} "
                f"(corr={correlation:.4f}, std={ratio_std:.4f}, "
...
# ----------------------------------------------------------------------
class PairTrader:
    """Manages trading for a single correlated pair"""
    def __init__(self, pair_info: dict, config: dict, interval: str, log: logging.Logger):
        ...
        self.ratio_mean = pair_info["mean"]
        self.ratio_std = pair_info["std"]
    ...
    def calculate_ratio(self, leader_price: float, follower_price: float) -> float:
        """Calculate price ratio between leader and follower"""
        return leader_price / follower_price
    
    def calculate_z_score(self, ratio: float) -> float:
        """Calculate z-score of ratio"""
        return (ratio - self.ratio_mean) / self.ratio_std if self.ratio_std > 0 else 0
    ...
    def execute_trade(self, signal: str, timestamp: pd.Timestamp, 
        ...
        if signal == "long":
            # Buy ratio: Buy leader, Sell follower
            self.position = "long"
            leader_side = "buy"
            follower_side = "sell"
        elif signal == "short":
            # Sell ratio: Sell leader, Buy follower
            self.position = "short"
            leader_side = "sell"
            follower_side = "buy"
        else:  # Exit
...
# ----------------------------------------------------------------------
# BACKTESTING ENGINE (MODIFIED FOR CORRELATION)
# ----------------------------------------------------------------------
class CorrelationBacktester:
    """Main backtesting engine for correlation strategy"""
    ...
    def run(self):
        ...
        for window in windows:
            ...
            try:
                _, valid_pairs = screen_correlated_pairs(
            ...
            required_keys = ["leader", "follower", "mean", "std"]
    ...
    def execute_trading_window(self, window: WindowPhase):
        ...
        for timestamp, row in window_data.iterrows():
            ...
            for pair, trader in list(self.active_traders.items()):
                ...
                price_ratio = trader.calculate_ratio(leader_price, follower_price)
                z_score = trader.calculate_z_score(price_ratio)
    ...
    def save_results(self):
        ...
        output_dir = self.repo_root / "reports" / "correlation" / self.interval
...
def main():
    parser = argparse.ArgumentParser(
        description="Correlation-based Statistical Arbitrage Backtester",
    ...
    try:
        ...
        log.info(f"Starting correlation backtest for {args.interval} interval")
        backtester = CorrelationBacktester(config, repo_root, args.interval, log)
        backtester.run()
